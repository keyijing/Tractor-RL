...
class ReplayBuffer:
	...
	def __init__(self, capacity, episode, seed=None):
	...
	def sample(self, batch_size): # only called by learner
		while self.size() < batch_size:
			time.sleep(0.1)
		assert len(self.buffer) > 0, "Empty buffer!"
		self.stats['sample_out'] += batch_size
		samples = self.rand.sample(self.buffer, batch_size)
		batch = self._pack(samples)
		return batch

	def pop(self, batch_size): # only called by learner
		while self.size() < batch_size:
			time.sleep(0.1)
		assert len(self.buffer) > 0, "Empty buffer!"
		self.stats['sample_out'] += batch_size
